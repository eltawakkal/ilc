...
class Auth
{
    ...
    function check_access($admin_role, $default_redirect=false, $redirect = false)
    {
        /*
        ...
        */
        ...
        $this->CI->db->select('admin_role');
        $this->CI->db->where('admin_id', $admin['id']);
        $this->CI->db->limit(1);
        $result = $this->CI->db->get('admin_tb');
        $result = $result->row();
    ...
    //  echo $result->access;
        if ($admin_role)
        {
            if ($admin_role == $result->admin_role)
            {
                return true;
            }
            else
    ...
    */
    function login_admin($username, $password, $remember=false)
    {
        ...
        $this->CI->db->select('*');
        $this->CI->db->where('admin_email', $username);
        ...
        if (sizeof($result) > 0)
        {
            $admin = array();
            $admin['admin'] = array();
            $admin['admin']['admin_id'] = $result['admin_id'];
			$admin['admin']['admin_guid'] = $result['admin_guid'];
            $admin['admin']['admin_name'] = $result['admin_name'];
            $admin['admin']['admin_email'] = $result['admin_email'];
            $admin['admin']['admin_username'] = $result['admin_username'];
			$admin['admin']['admin_role'] = $result['admin_role'];
			$admin['admin']['admin_img'] = $result['admin_img'];
			
            if($remember)
            {
                $loginCred = json_encode(array('admin_email'=>$username, 'admin_password'=>$password));
    ...
    function save($admin)
    {
       ...
       // {
        //    $this->CI->db->where('id', $admin['id']);
            $this->CI->db->update('admin_tb', $admin);
       // }
       // else
      //  {
    ...
    */
    ...
    {
        $this->CI->db->select('admin_username');
        $this->CI->db->from('admin_tb');
        $this->CI->db->where('admin_username', $str);
        if ($id)
        {
